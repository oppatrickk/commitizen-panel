...
// Vendored from microsoft/vscode: extensions/git/src/api/git.d.ts
// The built-in Git extension's API is not published as a package, so its type
// definitions have to be copied in. The only change from upstream is dropping the
// unused `SourceControlHistoryItem` import, which does not exist in @types/vscode 1.85.
